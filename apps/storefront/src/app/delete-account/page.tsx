...
export default function DeleteAccountPage() {
  ...
  const canSubmit =
    confirmText.trim().toUpperCase() === CONFIRM_WORD && !!password && !isBusy
  ...
  return (
    <div>
      ...
      <div className="mx-auto max-w-md p-4 md:px-6 md:pb-16">
        ...
        <p className="mt-1.5 text-sm leading-relaxed text-kudl-muted">
          This is permanent. There is no way to restore the account or its data
          afterwards.
        </p>
        ...
        <ul className="rounded-kudl-card border border-kudl-border bg-white p-4">
          ...
          <Bullet
            ...
            text={
              summary
                ? `${summary.addresses} saved ${
                    summary.addresses === 1 ? "address" : "addresses"
                  }`
        ...
        >
          ...
          >
            {isBusy ? (
              <Spinner className="h-5 w-5 text-white" label="Deleting" />
            ) : (
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="mb-2 mt-6 text-xs font-bold uppercase tracking-wide text-kudl-muted">
      {children}
    </h3>
  )
}
    ...
    <li className={`flex items-start gap-2.5 ${isLast ? "" : "pb-3"}`}>
      ...
      <span
        className={`text-[13px] leading-relaxed ${
          tinted ? "text-kudl-darker" : "text-kudl-body"
        }`}
      >
        {text}
      </span>
