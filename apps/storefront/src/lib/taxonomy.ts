 ...
 * (Dogs, Cats, Pharmacy and Shop By Breed sheets). This is catalogue-shaped
 * data, not Medusa data: the live seeded catalog (see productFacets.ts) is far
 * smaller than this taxonomy, so most of these categories/items will show a
 * (0) count until more products are seeded with matching metadata — same
 * accepted gap as the pre-existing brand list (see ShopByBrand.tsx).
 *
 * `slug` on every item/category is derived once here with slugify() so the
 * mega menu links and the filter sidebar checkboxes always agree on the same
 * value, instead of each independently slugging a label.
 */
...
const brandsOf = (menu: TaxonomyCategory[]): string[] =>
  menu.find((c) => c.category === "Popular Brands")?.items.map((i) => i.name) ?? []

export const allDogBrands = brandsOf(dogsMenu)
export const allCatBrands = brandsOf(catsMenu)
export const allBrands = Array.from(new Set([...allDogBrands, ...allCatBrands]))

export const findCategory = (
  menu: TaxonomyCategory[],
  categorySlug: string
): TaxonomyCategory | undefined => menu.find((c) => c.slug === categorySlug)

export const findItemLabel = (menu: TaxonomyCategory[], itemSlug: string): string | undefined => {
...
/** Item label plus the label of the taxonomy category (column) it lives under — used for breadcrumbs. */
export const findItemWithCategory = (
  menu: TaxonomyCategory[],
  itemSlug: string
): { categoryLabel: string; itemLabel: string } | undefined => {
  for (const cat of menu) {
    const item = cat.items.find((i) => i.slug === itemSlug)
    if (item) return { categoryLabel: cat.category, itemLabel: item.name }
  }
  return undefined
}
